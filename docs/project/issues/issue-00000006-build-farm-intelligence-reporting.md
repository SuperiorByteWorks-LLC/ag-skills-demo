...
**Known remaining repo-level checks**: `./scripts/ci-local.sh` still fails on two unrelated checks outside this NDVI scope: markdownlint reports multiple blank lines in `docs/research/data-migration-best-practices-2026.md`, and the link checker reports external TLS `UnknownIssuer` failures.
