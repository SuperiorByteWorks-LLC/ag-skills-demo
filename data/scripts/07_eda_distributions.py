...
Input:  data/soil/iowa_10_fields_soil.csv
        data/cdl/iowa_2023_cdl.csv, data/cdl/iowa_2024_cdl.csv
Output: data/EDA/iowa_soil_distributions.png
        data/EDA/iowa_cdl_distributions.png
"""
...
def main():
    ...
    ax.axvline(soil['ph1to1h2o_r'].mean(), color='red', linestyle='--', label=f'Mean: {soil["ph1to1h2o_r"].mean():.1f}')
    ...
    ax.axvline(soil['om_r'].mean(), color='red', linestyle='--', label=f'Mean: {soil["om_r"].mean():.1f}%')
    ...
    ax.axvline(soil['cec7_r'].mean(), color='red', linestyle='--', label=f'Mean: {soil["cec7_r"].mean():.1f}')
    ...
    ax.axvline(soil['claytotal_r'].mean(), color='red', linestyle='--', label=f'Mean: {soil["claytotal_r"].mean():.1f}%')
    ...
    ax.axvline(soil['awc_r'].mean(), color='red', linestyle='--', label=f'Mean: {soil["awc_r"].mean():.2f}')
    ...
    plt.suptitle('Soil Property Distributions Across All Fields', fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig('data/EDA/iowa_soil_distributions.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("✓ Saved: data/EDA/iowa_soil_distributions.png")
    ...
    # 2023 crops
    ax = axes[0]
    cdl_2023['crop_name'].value_counts().plot(kind='bar', ax=ax, color='steelblue', edgecolor='black')
    ...
    # 2024 crops
    ax = axes[1]
    cdl_2024['crop_name'].value_counts().plot(kind='bar', ax=ax, color='darkgreen', edgecolor='black')
    ...
    plt.tight_layout()
    plt.savefig('data/EDA/iowa_cdl_distributions.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("✓ Saved: data/EDA/iowa_cdl_distributions.png")
