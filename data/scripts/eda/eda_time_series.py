...
Input:  data/weather/iowa_10_fields_weather.csv
Output: data/EDA/iowa_weather_timeseries.png
        data/EDA/iowa_monthly_precip.png
        data/EDA/iowa_gdd_analysis.png
"""
...
def main():
    ...
    ax.fill_between(daily['date'], daily['T2M_MIN'], daily['T2M_MAX'], 
                   alpha=0.3, color='orange', label='Temperature Range')
    ax.plot(daily['date'], daily['T2M'], color='black', linewidth=1, label='Mean Temp')
    ax.plot(daily['date'], daily['T2M_MAX'], color='red', alpha=0.5, linewidth=0.5, label='Max Temp')
    ax.plot(daily['date'], daily['T2M_MIN'], color='blue', alpha=0.5, linewidth=0.5, label='Min Temp')
    ...
    plt.tight_layout()
    plt.savefig('data/EDA/iowa_weather_timeseries.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("✓ Saved: data/EDA/iowa_weather_timeseries.png")
    ...
    plt.tight_layout()
    plt.savefig('data/EDA/iowa_monthly_precip.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("✓ Saved: data/EDA/iowa_monthly_precip.png")
    ...
    for bar, val in zip(bars, gdd_values):
        ax2.annotate(f'{int(val)}', xy=(bar.get_x() + bar.get_width()/2, val),
                     xytext=(0, 5), textcoords="offset points", ha='center', fontsize=12, fontweight='bold')
    ax2.grid(True, alpha=0.3, axis='y')
    
    plt.tight_layout()
    plt.savefig('data/EDA/iowa_gdd_analysis.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("✓ Saved: data/EDA/iowa_gdd_analysis.png")
