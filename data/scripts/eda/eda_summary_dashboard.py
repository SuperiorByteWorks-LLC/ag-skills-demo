...
Output: data/EDA/iowa_summary_dashboard.png
"""
...
def main():
    ...
    fig.suptitle('Iowa Corn Belt Agricultural Analysis - Summary Dashboard', fontsize=20, fontweight='bold', y=0.98)
    ...
    ax2.bar(range(len(fields)), fields.sort_values('area_acres', ascending=False)['area_acres'], 
            color='steelblue', edgecolor='black')
    ax2.set_title('Field Sizes (sorted)', fontweight='bold')
    ax2.set_xlabel('Field Rank')
    ax2.set_ylabel('Acres')
    ax2.axhline(y=fields['area_acres'].mean(), color='red', linestyle='--', label=f'Mean: {fields["area_acres"].mean():.0f}')
    ax2.legend()
    ...
    ax4.fill_between(daily_avg['date'], 
    ...
    ax6.pie(drainage.values, labels=drainage.index, autopct='%1.0f%%', 
    ...
    plt.savefig('data/EDA/iowa_summary_dashboard.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("✓ Saved: data/EDA/iowa_summary_dashboard.png")
