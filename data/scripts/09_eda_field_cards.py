...
Output: data/EDA/field_cards/iowa_field_card_01.png through iowa_field_card_10.png
"""
...
def main():
    ...
    os.makedirs('data/EDA/field_cards', exist_ok=True)
    ...
    weather_summary = weather.groupby('field_id').agg({
        'T2M': 'mean',
        'PRECTOTCORR': 'sum',
    ...
    # Merge all data
    field_data = fields.merge(dominant_soil[['field_id', 'muname', 'compname', 'om_r', 'ph1to1h2o_r', 'cec7_r', 'claytotal_r', 'awc_r', 'drainagecl']], on='field_id')
    field_data = field_data.merge(weather_summary, on='field_id')
    field_data = field_data.merge(cdl_2023[['field_id', 'crop_name', 'dominant_pct']].rename(columns={'crop_name': 'cdl_2023', 'dominant_pct': 'cdl_2023_pct'}), on='field_id')
    field_data = field_data.merge(cdl_2024[['field_id', 'crop_name', 'dominant_pct']].rename(columns={'crop_name': 'cdl_2024', 'dominant_pct': 'cdl_2024_pct'}), on='field_id')
    ...
    for idx, field in field_data.iterrows():
        ...
        # Title
        fig.suptitle(f'Field Analysis Card: {field["field_id"][-6:]}', fontsize=18, fontweight='bold', y=0.98)
        ...
        soil_vals = [field['om_r'], field['ph1to1h2o_r']/2, field['cec7_r']/10, field['claytotal_r']/10]
        ...
        monthly_precip = field_weather.groupby(['year', 'month'])['PRECTOTCORR'].sum().unstack(level=0)
        ...
        if 2023 in monthly_precip.columns and 2024 in monthly_precip.columns:
            ax_precip.bar(x - width/2, monthly_precip[2023], width, label='2023', color='steelblue')
            ax_precip.bar(x + width/2, monthly_precip[2024], width, label='2024', color='darkgreen')
        ...
        ax_temp.plot(field_weather['date'], field_weather['T2M'], color='black', linewidth=1, label='Mean')
        ax_temp.fill_between(field_weather['date'], field_weather['T2M_MIN'], field_weather['T2M_MAX'], 
                           alpha=0.3, color='orange', label='Range')
        ...
        # Save card
        card_num = idx + 1
        plt.savefig(f'data/EDA/field_cards/iowa_field_card_{card_num:02d}.png', dpi=150, bbox_inches='tight')
        plt.close()
        
        print(f"✓ Card {card_num}/10: {field['field_id'][-6:]}")
    
    print(f"\n✓ All 10 field cards saved to: data/EDA/field_cards/")
