...
def _collect_peak_scene_rows(
    field_slug: str, *, crop_code: int, doy_window: tuple[int, int]
) -> list[dict[str, Any]]:
    ...
    for manifest_path in _sensor_manifest_paths(field_slug):
        ...
        for year_entry in manifest.get("years", []):
            ...
            if year is None:
                continue
            cdl_raster = _shared_cdl_raster_path(int(year))
...
def _collect_all_season_scene_rows(field_slug: str, boundary_path: Path) -> list[dict[str, Any]]:
...
def main() -> None:
    ...
    for _, field in fields.iterrows():
        ...
        seasonal_scene_rows = _collect_all_season_scene_rows(field_slug, boundary_path)
