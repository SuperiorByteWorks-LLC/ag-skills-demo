...
def plot_soil_properties_card(field_data: pd.DataFrame, field_id: str, output_path: Path) -> None:
    ...
    metrics = ['AWS (in)', 'CEC']
    values2 = [
        row.get('total_aws_inches', 0) or 0,
        row.get('avg_cec', 0) or 0
    ]
...
def plot_texture_triangle_card(field_data: pd.DataFrame, field_id: str, output_path: Path) -> None:
    ...
    if field_data.empty:
        ax.text(0.5, 0.5, "No soil data available", ha="center", va="center", transform=ax.transAxes)
        plt.savefig(output_path, dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
        plt.close(fig)
        return
...
def plot_farm_comparison_card(all_fields_data: dict[str, pd.DataFrame], output_path: Path) -> None:
    ...
    if not field_ids:
        for ax in axes.flat:
            ax.text(0.5, 0.5, "No valid soil data", ha="center", va="center", transform=ax.transAxes)
        plt.savefig(output_path, dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
        plt.close(fig)
        return
