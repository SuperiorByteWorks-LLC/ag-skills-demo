...
def build_field_reporting_dataset(
    ...
    df = compute_farm_relative_rankings(df)
...
def build_farm_summary(fields: gpd.GeoDataFrame, field_metrics: pd.DataFrame | None = None) -> pd.DataFrame:
...
def compute_management_implications(field_row: pd.Series | dict) -> list[str]:
    ...
    if ph is not None and not np.isnan(float(ph)):
        ph = float(ph)
        if ph < _PH_LOW:
            implications.append(f"pH {ph:.1f} is below optimum for corn/soy ({_PH_LOW}–{_PH_HIGH}); consider lime application")
        elif ph > _PH_HIGH:
    ...
    if om is not None and not np.isnan(float(om)):
        om = float(om)
        if om < _OM_LOW:
            implications.append(f"OM {om:.1f}% is low; prioritize cover crops or reduced tillage to rebuild organic matter")
        elif om >= _OM_HIGH:
            implications.append(f"OM {om:.1f}% is high for the region; strong water-holding and nutrient cycling capacity")

    aws = row.get("total_aws_inches")
    if aws is not None and not np.isnan(float(aws)):
        aws = float(aws)
        if aws < _AWS_LOW:
            implications.append(f"Total available water storage {aws:.1f} in is below 4 in; elevated drought sensitivity")
        else:
            implications.append(f"Available water storage {aws:.1f} in provides good buffer against short-term drought stress")
    ...
    if isinstance(drainage, str) and drainage:
        if any(w in drainage.lower() for w in ["poorly", "very poorly"]):
            implications.append(f"Drainage class '{drainage}' indicates wet soil conditions and potential trafficability constraints")
        elif "somewhat poorly" in drainage.lower():
            implications.append(f"Drainage class '{drainage}' may benefit from tile drainage review")
    ...
    if headlands_pct is not None and not np.isnan(float(headlands_pct)):
        ...
        if hp > _HEADLANDS_HIGH:
            implications.append(f"Headlands area {hp:.1f}% of field; high turning burden relative to productive acres")

    clay = row.get("avg_clay_pct")
    if clay is not None and not np.isnan(float(clay)):
        clay = float(clay)
        if clay > _CLAY_HIGH:
            implications.append(f"Clay content {clay:.0f}% is high; monitor compaction risk and spring trafficability")
    ...
    if diversity is not None and not np.isnan(float(diversity)):
        ...
        if diversity <= _CDL_LOW_DIVERSITY:
            implications.append("Crop history shows low rotation diversity; consider diversifying for disease and weed pressure management")

    erosion = row.get("erosion_risk")
    if isinstance(erosion, str) and "high" in erosion.lower():
        implications.append("Erosion risk is rated high; consider contour management or cover cropping on vulnerable slopes")
